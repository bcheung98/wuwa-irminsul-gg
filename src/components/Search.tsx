...
type Category = "Resonators" | "Weapons" | "Echoes";
...
function Search() {
    ...
    const echoes = [...useAppSelector(selectEchoes)].sort((a, b) =>
        a.displayName.localeCompare(b.displayName)
    );
    ...
    const data: Data[] = [
        ...characters.map((char) => ({
            ...
            category: "Resonators" as Category,
        })),
        ...weapons.map((wep) => ({
            name: wep.name,
...
function getImageIcon(category: Category, name: string) {
    switch (category) {
        ...
        case "Weapons":
            return `weapons/${name}`;
        case "Echoes":
            return `echoes/icons/${name.split(" ").join("_")}`;
    }
}
